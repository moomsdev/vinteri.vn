...
function app_get_permalink_title() {
	...
	return sprintf( __( 'Permanent Link to %s', 'laca' ), get_the_title() );
}
...
function app_get_index_404_message() {
	if ( is_category() ) {
		...
		return sprintf( __( 'Sorry, but there aren\'t any posts in the %s category yet.', 'laca' ), single_cat_title( '', false ) );
	}

	if ( is_date() ) {
		return __( 'Sorry, but there aren\'t any posts with this date.', 'laca' );
	}

	if ( is_author() ) {
		$userdata = get_user_by( 'id', get_queried_object_id() );
		/* translators: no posts found for author */
		return sprintf( __( 'Sorry, but there aren\'t any posts by %s yet.', 'laca' ), $userdata->display_name );
	}

	if ( is_search() ) {
		return __( 'No posts found. Try a different search?', 'laca' );
	}

	return __( 'No posts found.', 'laca' );
}
