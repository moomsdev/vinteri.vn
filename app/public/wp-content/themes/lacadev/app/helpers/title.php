...
function app_get_title() {
	...
	if ( is_home() || ( is_single() && get_post_type() === 'post' ) ) {
		...
		$title = sprintf( __( 'Search Results for: %s', 'laca' ), get_search_query() );
	} elseif ( is_category() ) {
		/* translators: category post listing page title */
		$title = sprintf( __( 'Category Archives: %s', 'laca' ), single_cat_title( '', false ) );
	} elseif ( is_tag() ) {
		/* translators: tag post listing page title */
		$title = sprintf( __( 'Tag Archives: %s', 'laca' ), single_tag_title( '', false ) );
	} elseif ( is_day() ) {
		...
		$title = __( 'Archives', 'laca' );
	} elseif ( is_404() ) {
		$title = __( 'Error 404 - Not Found', 'laca' );
	} elseif ( is_page() ) {
		$title = get_the_title();
	}
